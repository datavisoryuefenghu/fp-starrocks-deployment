...
# Deploy CRE-6630 Iceberg pipeline + StarRocks to useast1-preprod-a (preprod namespace).
#
...
#   bash environments/useast1-preprod-a/deploy.sh
...
echo "=== CRE-6630 Helm Deploy to ${NAMESPACE} on useast1-preprod-a ==="
...
SMT_URI=$(grep 'smtJarS3Uri' "${REPO_ROOT}/charts-iceberg/awsuseast1preproda/values.yaml" | awk -F'"' '{print $2}')
if [[ -z "$SMT_URI" ]]; then
  echo "ERROR: smtJarS3Uri is empty in charts-iceberg/awsuseast1preproda/values.yaml"
  exit 1
fi
...
helm upgrade --install fp-iceberg "${REPO_ROOT}/charts-iceberg" \
  -f "${REPO_ROOT}/charts-iceberg/awsuseast1preproda/values.yaml" \
  -n "${NAMESPACE}" \
...
helm upgrade --install fp-starrocks "${REPO_ROOT}/charts-starrocks" \
  -f "${REPO_ROOT}/charts-starrocks/awsuseast1preproda/values.yaml" \
  -n "${NAMESPACE}" \
  --wait --timeout "${TIMEOUT}"
...
echo ""
echo "Next: bash environments/useast1-preprod-a/verify.sh"
